...
class Matcher {
 public:
   ...
   * @return
   */
  std::vector<std::pair<int, int>> calculateCorrespondences(teaser::PointCloud& source_points,
                                                            ...
                                                            bool use_tuple_test         = true,
                                                            float tuple_scale           = 0,
                                                            bool use_optimized_matching = true);

  float thr_dist_ = 30;  // Empirically, the matching whose dist is larger than 30 is highly likely
                         // to be an outlier
  float num_max_corres_ =
      600;  // Empirically, too many correspondences cause slow down of the system
 ...
 private:
  ...
  template <typename T>
  void buildKDTreeWithTBB(const std::vector<T>& data, KDTree* tree);
  ...
  template <typename T>
  void searchKDTreeAll(Matcher::KDTree* tree,
